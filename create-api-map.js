...
// 配置正确的 json 接口
const swaggerUrl = 'http://resur-admin-test.meiquc.cn/resur-admin/v2/api-docs';
...
*/
export function ${it.methodName}(${data}): Promise<${resInterType ? 'IBasePage' : 'IBase'}<${resInterName}${resInterType}>> {
    return request({
...
function __mergeAndWrite(fileName, jsonStr) {
     ...
     * 文件名添加 Server 后缀
     */
    fs.writeFile(`${__fileDir}/${fileName}Server.ts`, jsonStr, (err) => {
        if (err) {
            console.error(err);
            return;
        }
...
function __requestParams(it, key, definitions, __fnInterface) {
    let bodyParams = [];
    if (definitions[R.pathOr('', ['body', 'ref'])(it)]) {
        bodyParams = R.mergeAll(definitions[it.body.ref].properties);
    } else if (R.pathOr(null, ['body'])(it)) {
        // TODO: 请求数组为基本类型，没有
        let ssType = R.path(['body', 'schema', 'items', 'type'])(it);
        if (it.body.schema.type == 'array' && (ssType == 'integer' || ssType == 'string')) {
            it.body.schema.items.isArray = true;
            bodyParams = { [it.body.name]: it.body.schema.items };
        }
    }
    let pathsAndFormdata = R.indexBy(R.prop('name'), [].concat(it.paths).concat(it.params));

    let requestParams = bodyParams;
    ...
    if (requestParams && !R.isEmpty(requestParams)) {
        ...
        if (__fnInterface[key].interfaceName.indexOf(interfaceName) !== -1) {
            return;
        }
        ...
        R.forEachObjIndexed((v, k, o) => {
            let __sType = v.type == 'integer' ? 'number' : v.type == 'string' ? 'string' : 'any';
            if (v.isArray) {
                __sType = __sType + '[]';
            }
            interfaceParamsStr += `    ${k}${v.required ? '' : '?'}: ${__sType}; /* ${v.description || ''}  ${v.format || ''}*/ \n`;
        }, requestParams);
        interfaceParamsStr += '}\n';
...
function __responseParams(responsesRef, methodName, key, definitions, __fnInterface, interfaceName = '') {
    ...
    R.forEachObjIndexed(async (v, k, o) => {
        ...
        if (o_ref) {
            ...
            interfaceParamsStr += `    ${k}${v.required ? '' : '?'}: ${v.type == 'integer' ? 'number' : v.type == 'string' ? 'string' : 'any'}, /* ${v.description || ''}  ${
                v.format || ''
            }*/ \n`;
            return;
        }
